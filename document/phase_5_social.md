...
* **Phase B — Friend-to-Friend Account Linking** *(done)*: A new `connections` table replaces free-text `friends` for the purpose of *linking* — users search by username/email, send/accept/reject connection requests, building a real social graph (`requesterId`, `addresseeId`, `status: pending|accepted|rejected|blocked`). See [phase_b_social_profile.md](file:///d:/Project/My-Mini-Projects/M.A.D%28Money%20Analysis%20and%20Decisions%29/document/phase_b_social_profile.md).
* **Phase C — Shared Expense Schema + Migration** *(done)*: The one-directional `splits` table (no `userId` column at all — settlement was guarded only by `friendId`) is replaced by `expenses` + `expense_participants` (defined in [db.js](file:///d:/Project/My-Mini-Projects/M.A.D%28Money%20Analysis%20and%20Decisions%29/backend/db.js)), a bidirectionally-visible model where one row is queryable from either party's side via `expenses.payerId` and `expense_participants.userId`. A one-off migration script ([migrate_shared_expenses.js](file:///d:/Project/My-Mini-Projects/M.A.D%28Money%20Analysis%20and%20Decisions%29/backend/migrate_shared_expenses.js)) carries forward the live `friends`/`splits` data as `legacyFriendName`-tagged, `isLocalOnly` "local only" entries so nothing is lost. Run it once after deploying with `node migrate_shared_expenses.js`. The legacy `friends`/`splits` tables and routes are untouched for now — Phase D will switch the Split Picker and ledger over to the new schema.
